...
void console_thread_entry(ULONG thread_input)
{
    ...
    ssp_err = p_console->p_api->open(p_console->p_ctrl, p_console->p_cfg);
    if(SSP_SUCCESS != ssp_err)
    {
        SEGGER_RTT_printf(0, "Failed console_thread_entry::g_sf_console0.p_api->open, ssp_err = %d\r\n", ssp_err);
    }

    ssp_err = p_console->p_api->write(p_console->p_ctrl, (uint8_t *)"\r\nWelcome to Grutter's example ThreadX System Developer\r\nEnter '?' for a list of commands...\r\n", 100);
    if(SSP_SUCCESS != ssp_err)
    {
        SEGGER_RTT_printf(0, "Failed console_thread_entry::g_sf_console0.p_api->write, ssp_err = %d\r\n", ssp_err);
    }

    while(1)
    {
        /* This version blocks, so its unclear if it is sleeping or not */
        ssp_err = p_console->p_api->prompt(p_console->p_ctrl, &gp_console->sf_console_menu, TX_WAIT_FOREVER);
        if((SSP_SUCCESS != ssp_err) && (SSP_ERR_TIMEOUT != ssp_err))
        {
            SEGGER_RTT_printf(0, "Failed console_thread_entry::g_sf_console0.p_api->prompt, ssp_err = %d\r\n", ssp_err);
        }
    }
}
