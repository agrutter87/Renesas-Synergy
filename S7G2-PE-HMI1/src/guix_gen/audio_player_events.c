...
UINT w_splash_event (GX_WINDOW * widget, GX_EVENT * event_ptr)
{
    ...
    switch (event_ptr->gx_event_type)
    {
        case GX_EVENT_SHOW:
        {
            status = gx_prompt_text_set(&w_splash.w_splash_txt_version, APP_VERSION);
        ...
        case GX_EVENT_TIMER:
        {
            status = guix_show_window((GX_WINDOW*)&w_main, (GX_WIDGET *) widget, true);
            break;
        }
...
/** Files Screen Event Handler */
UINT w_files_event (GX_WINDOW * widget, GX_EVENT * event_ptr)
{
    UINT                     status;
    ...
    switch (event_ptr->gx_event_type)
    {
        ...
        case GX_EVENT_FOCUS_GAINED:
        {
            ...
            status = gx_progress_bar_pixelmap_set(&w_main.w_main_bar_volume, GX_PIXELMAP_ID_BAR_VOL);
            APP_ERROR_TRAP(status);
            status = gx_progress_bar_value_set(&w_main.w_main_bar_volume, g_player_status.volume);
            APP_ERROR_TRAP(status);
        ...
        case GX_SIGNAL(ID_BTN_LEFT, GX_EVENT_CLICKED):
        {
            switch (g_player_status.state)
            {
                ...
                {
                    if (!g_file_list[g_file_list_index + 5][0])
                    {
                        break;
                    }
        ...
        case GX_SIGNAL(ID_BTN_MID, GX_EVENT_CLICKED):
        {
            switch (g_player_status.state)
            {
                case SF_AUDIO_PLAYBACK_STATUS_STOPPED:
                {
                    ...
                    status        = gx_vertical_list_selected_index_get(&w_main.w_main_list_files, &g_file_index);
                    APP_ERROR_TRAP(status)
        ...
        case APP_EVENT_ERROR:
        {
            switch (event_ptr->gx_event_payload.gx_event_ulongdata)
            {
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_OPEN);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_PAUSE);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_RESUME);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_CLOSE);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_VOLUME);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_PLAYBACK);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_NOT_RIFF);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_NOT_WAVE);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_NOT_PCM);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_NOT_MONO);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_NOT_44KHZ);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_NOT_16BIT);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_ERR_NO_DATA_CHUNK);
                    APP_ERROR_TRAP(status)
            ...
            status = gx_system_timer_start((GX_WIDGET *) &w_main.w_main_w_files, APP_TIMER_STATUS, 150, 0);
            APP_ERROR_TRAP(status)
        ...
        case GX_EVENT_TIMER:
        {
            switch (event_ptr->gx_event_payload.gx_event_timer_id)
            {
                case APP_TIMER_PLAYBACK:
                {
                    ...
                    status = gx_prompt_text_set(&w_main.w_main_txt_time, txt_buffer);
                    APP_ERROR_TRAP(status)
                ...
                {
                    status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_STOPPED);
                    APP_ERROR_TRAP(status)

                    gx_system_timer_stop((GX_WIDGET *) &w_main.w_main_w_files, APP_TIMER_STATUS);
...
UINT w_navbar_event (GX_WINDOW * widget, GX_EVENT * event_ptr)
{
    switch (event_ptr->gx_event_type)
    {
        case GX_EVENT_SHOW:
        	/* Only process GX_EVENT_SHOW */
        	return gx_window_event_process(widget, event_ptr);
...
static UINT app_gx_refresh_file_names (UINT first_index)
{
    ...
    if(!strcmp(g_file_list[first_index], ""))
    {
    	strcpy(file_name[0], "Insert USB mass storage device");
    	gx_vertical_list_selected_set(&w_main.w_main_list_files, 0);
    }

    status = gx_prompt_text_set(&w_main.w_main_list_item_0, file_name[0]);
    APP_ERROR_RETURN(status)
    status = gx_prompt_text_set(&w_main.w_main_list_item_1, file_name[1]);
    APP_ERROR_RETURN(status)
    status = gx_prompt_text_set(&w_main.w_main_list_item_2, file_name[2]);
    APP_ERROR_RETURN(status)
    status = gx_prompt_text_set(&w_main.w_main_list_item_3, file_name[3]);
    APP_ERROR_RETURN(status)
    status = gx_prompt_text_set(&w_main.w_main_list_item_4, file_name[4]);
    APP_ERROR_RETURN(status)
...
static UINT app_gx_set_controls (void)
{
    ...
    switch (g_player_status.state)
    {
        case SF_AUDIO_PLAYBACK_STATUS_STOPPED:
        {
            status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_STOPPED);
            APP_ERROR_RETURN(status)
            status = gx_prompt_text_id_set(&w_main.w_main_txt_time, GX_STRING_ID_BLANK);
            APP_ERROR_RETURN(status)
            status = gx_pixelmap_button_pixelmap_set(&w_main.w_main_btn_left,
                                                     GX_PIXELMAP_ID_BTN_DOWN, 0, 0);
            APP_ERROR_RETURN(status)
            status = gx_pixelmap_button_pixelmap_set(&w_main.w_main_btn_mid,
                                                     GX_PIXELMAP_ID_BTN_CIRCLE, 0, 0);
            APP_ERROR_RETURN(status)
            status = gx_pixelmap_button_pixelmap_set(&w_main.w_main_btn_right,
                                                     GX_PIXELMAP_ID_BTN_UP, 0, 0);
            APP_ERROR_RETURN(status)

            gx_system_timer_stop((GX_WIDGET *) &w_main.w_main_w_files, APP_TIMER_PLAYBACK);
            gx_system_timer_stop((GX_WIDGET *) &w_main.w_main_w_files, APP_TIMER_STATUS);
        ...
        case SF_AUDIO_PLAYBACK_STATUS_PAUSED:
        {
            status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_PAUSED);
            APP_ERROR_RETURN(status)
            status = gx_pixelmap_button_pixelmap_set(&w_main.w_main_btn_left,
        ...
        case SF_AUDIO_PLAYBACK_STATUS_PLAYING:
        {
            status = gx_prompt_text_id_set(&w_main.w_main_txt_status, GX_STRING_ID_PLAYING);
            APP_ERROR_RETURN(status)
            status = gx_pixelmap_button_pixelmap_set(&w_main.w_main_btn_left,
            ...
            status = gx_system_timer_start((GX_WIDGET *) &w_main.w_main_w_files, APP_TIMER_PLAYBACK, 5, 5);
            APP_ERROR_TRAP(status)
