...
#define SF_CONSOLE_ERROR_RETURN(a, err)  SSP_ERROR_RETURN((a), (err), &g_module_name[0], &g_sf_console_version)
#endif
...
static ssp_err_t check_for_overflow(sf_console_instance_ctrl_t * const p_ctrl, uint32_t * p_index, uint32_t const bytes);
static ssp_err_t insert_char(sf_console_instance_ctrl_t * const p_ctrl,
                             uint8_t                    * const p_input,
                             uint8_t                            ch,
                             uint32_t                           index,
                             uint32_t                           last_index);
static ssp_err_t delete_char(sf_console_instance_ctrl_t * p_ctrl, uint8_t * p_input, uint32_t index, uint32_t * p_last_index);
static void move_cursor(sf_console_instance_ctrl_t * p_ctrl, sf_console_cursor_dir_t dir, uint32_t num_spaces);
static ssp_err_t print_help_menu(sf_console_instance_ctrl_t * const p_ctrl,
...
static void sf_console_call_callback(sf_console_instance_ctrl_t * const p_ctrl,
                                     ...
                                     uint32_t                     const bytes,
                                     int32_t                            length);
static ssp_err_t sf_console_read_process_up_arrow(sf_console_instance_ctrl_t * const p_ctrl,
                                             uint8_t                    * const p_dest,
                                             uint32_t                   *       p_index,
                                             uint32_t                   *       p_length);
static ssp_err_t sf_console_read_process_escape(sf_console_instance_ctrl_t * const p_ctrl,
                                                uint8_t                    * const p_dest,
                                                uint32_t                   *       p_index,
                                                uint32_t                   *       p_length);
static ssp_err_t sf_console_read_process_backspace(sf_console_instance_ctrl_t * const p_ctrl,
                                                   uint8_t                    * const p_dest,
                                                   uint32_t                   *       p_index,
                                                   uint32_t                   *       p_length);
static bool sf_console_read_process_new_line(sf_console_instance_ctrl_t * const p_ctrl,
                                             ...
                                             uint32_t                           length,
                                             uint8_t                      const rx);
static ssp_err_t sf_console_read_main(sf_console_instance_ctrl_t * const p_ctrl,
                                      uint8_t                    * const p_dest,
                                      uint32_t                     const bytes,
                                      uint32_t                     const timeout);

#if SF_CONSOLE_CFG_PARAM_CHECKING_ENABLE
static ssp_err_t sf_console_parse_param_check(sf_console_instance_ctrl_t * const p_ctrl,
                                              sf_console_menu_t const * const p_menu,
                                              uint8_t           const * const p_input);
#endif
...
static const ssp_version_t g_sf_console_version =
{
    .api_version_major = SF_CONSOLE_API_VERSION_MAJOR,
    .api_version_minor = SF_CONSOLE_API_VERSION_MINOR,
    .code_version_major = SF_CONSOLE_CODE_VERSION_MAJOR,
    .code_version_minor = SF_CONSOLE_CODE_VERSION_MINOR,
};
#if defined(__GNUC__)
...
const sf_console_api_t g_sf_console_on_sf_console = 
{
    ...
    .versionGet   = SF_CONSOLE_VersionGet
};
/*LDRA_ANALYSIS */
 ...
 *
 * @retval  SSP_SUCCESS          Console channel is successfully opened
...
ssp_err_t SF_CONSOLE_Open (sf_console_ctrl_t * const p_api_ctrl, sf_console_cfg_t const * const p_cfg)
{
    ...
    SSP_ASSERT(NULL != p_cfg->p_initial_menu);
    SSP_ASSERT(NULL != p_cfg->p_comms);
    SSP_ASSERT(NULL != p_cfg->p_comms->p_api);
    SSP_ASSERT(NULL != p_cfg->p_comms->p_api->open);
#endif

    /** Open UART driver */
    ssp_err_t err;
    err = p_cfg->p_comms->p_api->open(p_cfg->p_comms->p_ctrl, p_cfg->p_comms->p_cfg);
    SF_CONSOLE_ERROR_RETURN(SSP_SUCCESS == err, err);
 ...
 *
 * @retval SSP_SUCCESS           Console successfully closed
 ...
 *
 * @retval SSP_SUCCESS           Data parsed successfully, command found and callback called.
 * @retval SSP_ERR_UNSUPPORTED   Command not found in the current menu.
 * @retval SSP_ERR_ASSERTION     One or more input parameter pointers are invalid.
 * @return                       See @ref Common_Error_Codes or lower level drivers for other possible return codes.
 * @note   This function is reentrant for any channel.
***********************************************************************************************************************/
                           ...
                           uint32_t                  const bytes)
{
...
#if SF_CONSOLE_CFG_PARAM_CHECKING_ENABLE
    /* Parameter check.*/
    if(SSP_SUCCESS != sf_console_parse_param_check(p_ctrl, p_menu, p_input))
    {
    ...
    SSP_ERROR_LOG((SSP_ERR_UNSUPPORTED), (g_module_name), (g_sf_console_version));
 ...
 *
 * @retval SSP_SUCCESS           Received valid command and called callback
                            ...
                            uint32_t            const timeout)
{
    ...
    SSP_ASSERT(NULL != p_ctrl);
    SSP_ASSERT(NULL != p_ctrl->p_comms);
    SSP_ASSERT(NULL != p_ctrl->p_comms->p_api);
    SSP_ASSERT(NULL != p_ctrl->p_comms->p_api->lock);
    SSP_ASSERT(NULL != p_ctrl->p_comms->p_api->unlock);
#endif
 ...
 * @retval SSP_ERR_INVALID_SIZE  If length passed is zero or length passed is greater than 128U (max value).
 * @return                       See @ref Common_Error_Codes or lower level drivers for other possible return codes.
 * @note This function is reentrant for any channel.
***********************************************************************************************************************/
                            ...
                            uint32_t         const timeout)
{
    ...
    SF_CONSOLE_ERROR_RETURN(0U != length, SSP_ERR_INVALID_SIZE);
    SF_CONSOLE_ERROR_RETURN(length < SF_CONSOLE_MAX_WRITE_LENGTH, SSP_ERR_INVALID_SIZE);
    ssp_err_t err = p_ctrl->p_comms->p_api->write(p_ctrl->p_comms->p_ctrl, p_src, length, timeout);
    SF_CONSOLE_ERROR_RETURN(SSP_SUCCESS == err, err);
 ...
 *
 * @retval SSP_SUCCESS           Argument found successfully
 * @retval SSP_ERR_ASSERTION     p_arg or p_str is NULL
 * @retval SSP_ERR_INTERNAL      String passed after parsing command is NULL_CODE.
 * @return                       See @ref Common_Error_Codes or lower level drivers for other possible return codes.
 * @note This function is reentrant for any channel.
***********************************************************************************************************************/
...
{
#if SF_CONSOLE_CFG_PARAM_CHECKING_ENABLE
    SSP_ASSERT(NULL != p_arg);
    SSP_ASSERT(NULL != p_str);
#endif
...
/******************************************************************************************************************//**
* @brief Console version get function.
*
* @param[in] p_version  Version information stored here.
*
* @retval SSP_SUCCESS        Version stored in provided pointer.
...
ssp_err_t SF_CONSOLE_VersionGet (ssp_version_t * const p_version)
{
    ...
    /** Set version pointer */
    p_version->version_id = g_sf_console_version.version_id;

    return SSP_SUCCESS;
}  /* End of function SF_CONSOLE_VersionGet() */
...
* @retval         SSP_SUCCESS   Deleting characters from console is processed successfully.
* @return                       See @ref Common_Error_Codes or lower level drivers for other possible return codes
***********************************************************************************************************************/
static ssp_err_t delete_char(sf_console_instance_ctrl_t * p_ctrl, uint8_t * p_input, uint32_t index, uint32_t * p_last_index)
{
    ...
    if (index < last_index)
    {
        ...
        *p_last_index = last_index;
        if ((0U == last_index) && (p_ctrl->echo))
        {
            ...
            ssp_err_t err = p_ctrl->p_comms->p_api->unlock(p_ctrl->p_comms->p_ctrl, SF_COMMS_LOCK_TX);
            SF_CONSOLE_ERROR_RETURN(SSP_SUCCESS == err, err);
        }
    }
...
* @retval     SSP_SUCCESS  Character has been inserted or processed successfully.
* @return                  See @ref Common_Error_Codes or lower level drivers for other possible return codes
***********************************************************************************************************************/
static ssp_err_t insert_char(sf_console_instance_ctrl_t * const p_ctrl,
                             uint8_t                    * const p_input,
...
* @retval SSP_SUCCESS     Prints help menu to console successfully.
* @return                 See @ref Common_Error_Codes or lower level drivers for other possible return codes
***********************************************************************************************************************/
static ssp_err_t print_help_menu(sf_console_instance_ctrl_t * const p_ctrl, sf_console_menu_t const * const p_menu,
        UINT timeout)
{
...
* @retval         SSP_SUCCESS  Up arrow processed successfully.
* @return                      See @ref Common_Error_Codes or lower level drivers for other possible return codes.
***********************************************************************************************************************/
static ssp_err_t sf_console_read_process_up_arrow(sf_console_instance_ctrl_t * const p_ctrl,
                                             uint8_t                    * const p_dest,
...
* @param[in,out]  p_index           The cursor index in the destination buffer
* @param[in,out]  p_length          The length of the destination buffer (in bytes)
* @retval         SSP_SUCCESS       Received Escape code processed successfully.
* @retval         SSP_ERR_INTERNAL  Incorrect Escape code received.
***********************************************************************************************************************/
static ssp_err_t sf_console_read_process_escape(sf_console_instance_ctrl_t * const p_ctrl,
                                                uint8_t                    * const p_dest,
                                                uint32_t                   *       p_index,
                                                uint32_t                   *       p_length)
{
    ...
    SF_CONSOLE_ERROR_RETURN(SSP_SUCCESS == err, err);
    SF_CONSOLE_ERROR_RETURN(ESC_CODE_2 == rxbuf[0], SSP_ERR_INTERNAL);
...
* @param[in,out]  p_length    The length of the destination buffer (in bytes)
* @retval         SSP_SUCCESS Backspace input to the console is processed successfully.
***********************************************************************************************************************/
static ssp_err_t sf_console_read_process_backspace(sf_console_instance_ctrl_t * const p_ctrl,
                                              uint8_t                    * const p_dest,
                                              uint32_t                   *       p_index,
                                              uint32_t                   *       p_length)
{
    ...
    if(0U != *p_index)
    {
        ...
        ssp_err_t err = delete_char(p_ctrl, p_dest, *p_index, p_length);
        SF_CONSOLE_ERROR_RETURN(SSP_SUCCESS == err, err);
    }
...
*
* @retval     SSP_SUCCESS       All the input parameter pointers are valid.
* @retval     SSP_ERR_ASSERTION One or more input parameter pointers are invalid.
***********************************************************************************************************************/
static ssp_err_t sf_console_parse_param_check(sf_console_instance_ctrl_t * const p_ctrl,
                                              sf_console_menu_t const * const p_menu,
...
{
    SSP_ASSERT(NULL != p_ctrl);
    SSP_ASSERT(NULL != p_menu);
    SSP_ASSERT(NULL != p_input);
...
* @retval         SSP_SUCCESS        One input byte is processed successfully.
***********************************************************************************************************************/
static ssp_err_t sf_console_read_process_byte(sf_console_instance_ctrl_t * const p_ctrl,
        uint8_t                    * const p_dest,
        uint8_t                      const rx,
        uint32_t                   *       p_index,
        uint32_t                   *       p_length,
...
* @retval     SSP_SUCCESS       One byte of data is read successfully.
* @retval     SSP_ERR_OVERFLOW  Input buffer is overflowed.
* @return                       See @ref Common_Error_Codes or lower level drivers for other possible return codes.
***********************************************************************************************************************/
static ssp_err_t sf_console_read_main(sf_console_instance_ctrl_t * const p_ctrl,
                                      uint8_t                    * const p_dest,
